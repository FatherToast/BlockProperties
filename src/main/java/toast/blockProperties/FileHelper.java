...
public abstract class FileHelper {
    ...
    // The main directory for block properties.
    ...
    public static void init(File directory) {
        ...
        FileHelper.PROPS_DIRECTORY = new File(directory, "BlockProperties");
        FileHelper.PROPS_DIRECTORY.mkdirs();
        FileHelper.EXTERNAL_DIRECTORY = new File(directory, "BlockPropertiesExternal");
        FileHelper.EXTERNAL_DIRECTORY.mkdirs();
        FileHelper.SCHEMATIC_DIRECTORY = new File(directory, "BlockPropertiesSchematics");
    ...
    public static File generateNbtStats(String entityName, NBTTagCompound tag) {
        ...
        try {
            ...
            if (propFile.exists()) {
                ...
                if (attempt > 999) {
                    _BlockPropertiesMod.console("[WARNING] Too many similar files for \"" + entityName + "\"! Cancelled external nbt function generator.");
                    return null;
                }
                fileName += attempt;
            }
            ...
            _BlockPropertiesMod.console("[WARNING] Failed to generate external nbt function for \"" + entityName + "\": " + tag.toString());
            ex.printStackTrace();
        }
        return null;
    }
    ...
    // Loads all block properties into memory.
    public static int load() {
        int filesLoaded = 0;
        String[] types = { "drops", "harvest", "items", "nbt", "speed", "xp" };
        File externalDir;
    ...
    // Recursively loads the block properties in the given directory.
    private static int loadDirectory(File directory) {
        ...
        for (File propFile : directory.listFiles(new ExtensionFilter(FileHelper.FILE_EXT))) {
            ...
            BlockProperties.load(propFile.getPath(), node);
            filesLoaded++;
        }
    ...
    private static JsonObject loadFile(File propFile) {
        ...
        try {
            ...
            throw new BlockPropertyException("Error reading file! (See \"Caused by:\" below for more info.)", propFile.getPath(), ex);
        }
        if (node == null)
            throw new BlockPropertyException("Failed to read file", propFile.getPath());
        if (!node.isJsonObject())
            throw new BlockPropertyException("Invalid file! (non-object)", propFile.getPath());
	...
	private static NBTTagCompound loadNBTFile(File nbtFile) {
        try {
            ...
            throw new BlockPropertyException("Error reading nbt file!", nbtFile.getPath(), ex);
        }
        throw new BlockPropertyException("Failed to read nbt file!", nbtFile.getPath());
    }
    ...
    private static NBTTagCompound loadUncompressedNBTFile(File nbtFile) {
        try {
            ...
            throw new BlockPropertyException("Error reading uncompressed nbt file!", nbtFile.getPath(), ex);
        }
        throw new BlockPropertyException("Failed to read uncompressed nbt file!", nbtFile.getPath());
    }
    ...
    public static int generateDefaults() {
        ...
        File defaultPropFile = new File(FileHelper.CONFIG_DIRECTORY, "BlockProperties.json");
        if (!defaultPropFile.exists()) {
            defaultProps = new JsonObject();
            defaultProps.addProperty("_comment", "This is the default Block Properties file. When this mod generates a default file for any block, it will be an auto-formatted copy of this file. Remember, comments outside of functions (such as this one) will not be copied.");
            defaultProps.add("stats", new JsonObject());
            defaultProps.add("break_speed", new JsonArray());
            defaultProps.add("drops", new JsonArray());
            defaultProps.add("harvest", new JsonArray());
            defaultProps.add("xp", new JsonArray());
            try {
                ...
                _BlockPropertiesMod.console("[WARNING] Failed to generate the default properties file!");
            }
        }
        else {
        ...
        for (String blockId : new HashSet<String>(Block.blockRegistry.getKeys())) {
            if (BlockProperties.getProperties(blockId) == null && !blockId.equals("minecraft:air")) {
                ...
                try {
                    ...
                    if (propFile.exists()) {
                        ...
                        if (attempt > 99) {
                            _BlockPropertiesMod.console("[WARNING] Failed to generate default properties file for \"" + blockId + "\"!");
                            continue;
                        }
                        fileName += attempt;
                    }
                    props = new JsonObject();
                    props.addProperty("_name", blockId);
                    ...
                    if (defaultProps.has("stats")) {
                        props.add("stats", BlockStats.populateDefaults(defaultProps.getAsJsonObject("stats"), Block.getBlockFromName(blockId)));
                    }
                    propFile.createNewFile();
                    FileWriter out = new FileWriter(propFile);
                    FileHelper.GSON_PRETTY.toJson(props, out);
                    out.close();
                    filesGenerated++;
                    BlockProperties.load(propFile.getPath(), props);
                }
                catch (BlockPropertyException ex) {
                    throw ex;
                }
        ...
        try {
            ...
            for (String name : required)
                if (!fieldNames.contains(name))
                    throw new BlockPropertyException("Verify error! Missing required field \"" + name + "\". (Required fields: " + Arrays.toString(requiredFields) + ")", path);
            for (String name : fieldNames)
                if (!allowed.contains(name))
                    throw new BlockPropertyException("Verify error! Invalid field \"" + name + "\". (Allowed fields: " + Arrays.toString(allowed.toArray(new String[0])) + ")", path);
        }
        catch (IllegalStateException ex) {
            throw new BlockPropertyException("Verify error! (must be an object)", path);
        }
    }
    ...
    public static void verify(JsonObject node, String path, IProperty property) {
        ...
        allowed.addAll(optional);
        allowed.add("_comment");
        if (! (property instanceof BlockProperties)) {
        ...
        try {
            ...
            for (String name : required)
                if (!fieldNames.contains(name))
                    throw new BlockPropertyException("Verify error! Missing required field \"" + name + "\". (Required fields: " + Arrays.toString(property.getRequiredFields()) + ")", path);
            for (String name : fieldNames)
                if (!allowed.contains(name))
                    throw new BlockPropertyException("Verify error! Invalid field \"" + name + "\". (Allowed fields: " + Arrays.toString(allowed.toArray(new String[0])) + ")", path);
        }
        catch (IllegalStateException ex) {
            throw new BlockPropertyException("Verify error! (functions must be objects)", path);
        }
    }
    ...
    public static String getFunctionString(JsonObject node, String path) {
        try {
            ...
            throw new BlockPropertyException("Error generating function string!", path, ex);
        }
    }
    ...
    public static JsonObject loadFunctionFromString(String path, String prop, int index) {
        try {
            ...
            try {
                ...
                throw new BlockPropertyException("Error loading function string! " + prop, path, ex);
            }
            if (node == null)
                throw new BlockPropertyException("Failed to load function string! " + prop, path);
            if (!node.isJsonObject())
                throw new BlockPropertyException("Invalid function string! (non-object) " + prop, path);
    ...
    public static double getValue(double[] values) {
        return FileHelper.getCount(values, _BlockPropertiesMod.random);
    }
    ...
    public static int getCount(double[] counts) {
        return FileHelper.getCount(counts, _BlockPropertiesMod.random);
    }
    ...
    public static long getLongCount(double[] counts) {
        return FileHelper.getLongCount(counts, _BlockPropertiesMod.random);
    }
    ...
    public static String readText(JsonObject node, String path, String tag, String defaultValue) {
        try {
            ...
            throw new BlockPropertyException("Invalid value for \"" + tag + "\"! (wrong node type)", path);
        }
    }
    ...
    public static boolean readBoolean(JsonObject node, String path, String tag, boolean defaultValue) {
        ...
        if (text.equals("false"))
            ...
            return true;
        throw new BlockPropertyException("Invalid boolean value! (" + text + ": must be true or false)", path);
    }
    ...
    public static double[] readCounts(JsonObject node, String path, String tag, int index, double defaultMin, double defaultMax) {
        ...
        try {
            ...
            throw new BlockPropertyException("Unexpected error! (array index out of bounds)", path);
        }
        catch (Exception ex) {
            throw new BlockPropertyException("Invalid number range! (wrong node type)", path);
        }
    ...
    private static double[] readCounts(String value, String path) {
        ...
        try {
            ...
            throw new BlockPropertyException("Invalid number range! (" + subParts[0].trim() + ")", path);
        }
        if (subParts.length == 1) {
            ...
            try {
                ...
                throw new BlockPropertyException("Invalid number range! (" + subParts[1].trim() + ")", path);
            }
        }
        else
            throw new BlockPropertyException("Invalid number range! (too many \'~\'s)", path);
        if (Double.isNaN(counts[0]) || Double.isNaN(counts[1]) || Double.isInfinite(counts[0]) || Double.isInfinite(counts[1]))
            throw new BlockPropertyException("Invalid number range! (NaN and Infinity are not allowed)", path);
    ...
    public static int readWeight(JsonObject node, String path, int defaultValue) {
        ...
        try {
            ...
            if (weight < 0)
                throw new BlockPropertyException("Invalid weight! (" + value + ": must be non-negative)", path);
            return weight;
        }
        catch (NumberFormatException ex) {
            throw new BlockPropertyException("Invalid weight! (" + value + ")", path, ex);
        }
    }
    ...
    public static int readInteger(JsonObject node, String path, String tag, int defaultValue) {
        ...
        try {
            ...
            throw new BlockPropertyException("Invalid integer! (wrong node type)", path);
        }
        ...
        try {
            ...
            throw new BlockPropertyException("Invalid integer! (" + value + ")", path, ex);
        }
    }
    ...
    public static double readDouble(JsonObject node, String path, String tag, double defaultValue) {
        ...
        try {
            ...
            throw new BlockPropertyException("Invalid double! (wrong node type)", path);
        }
        ...
        try {
            ...
            throw new BlockPropertyException("Invalid double! (" + value + ")", path, ex);
        }
    }
    ...
    public static Item readItem(String id, String path, boolean required) {
        ...
        if (item == null) {
            try {
                ...
                if (item != null) {
                    _BlockPropertiesMod.console("[WARNING] Usage of numerical item id! (" + id + "=\"" + Item.itemRegistry.getNameForObject(item) + "\") at " + path);
                }
            }
            catch (NumberFormatException ex) {
                // Do nothing
        ...
        if (required && item == null)
            throw new BlockPropertyException("Missing or invalid item!", path);
        return item;
    }
    ...
    public static Block readBlock(String id, String path, boolean required) {
        ...
        if (block == null || block == Blocks.air) {
            try {
                ...
                if (block != null && block != Blocks.air) {
                    _BlockPropertiesMod.console("[WARNING] Usage of numerical block id! (" + id + "=\"" + Block.blockRegistry.getNameForObject(block) + "\") at " + path);
                }
            }
            catch (NumberFormatException ex) {
                // Do nothing
        ...
        if (required && (block == null || block == Blocks.air))
            throw new BlockPropertyException("Missing or invalid block!", path);
        return block;
    }
    ...
    public static Potion readPotion(String id, String path, boolean required) {
    	...
    	if (potion == null) {
    		try {
                ...
                if (potion != null) {
                    _BlockPropertiesMod.console("[WARNING] Usage of numerical potion id! (" + id + "=\"" + potion.getName() + "\") at " + path);
                }
            }
        ...
        if (required && potion == null)
			throw new BlockPropertyException("Missing or invalid potion! (" + id + ")", path);
        return potion;
    }
    ...
    public static Enchantment readEnchant(String id, String path, boolean required) {
    	...
    	if (enchant == null) {
    		try {
                ...
                if (enchant != null) {
                    _BlockPropertiesMod.console("[WARNING] Usage of numerical enchantment id! (" + id + "=\"" + enchant.getName() + "\") at " + path);
                }
            }
            catch (ArrayIndexOutOfBoundsException ex) {
                // Do nothing
        ...
        if (required && enchant == null)
			throw new BlockPropertyException("Missing or invalid enchantment! (" + id + ")", path);
        return enchant;
    }
