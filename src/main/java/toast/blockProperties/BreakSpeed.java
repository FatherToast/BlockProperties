...
import toast.blockProperties.entry.misc.EntrySpeed;
import toast.blockProperties.entry.misc.EntrySpeedCopy;

import com.google.gson.JsonArray;
import com.google.gson.JsonElement;
import com.google.gson.JsonObject;

public class BreakSpeed implements IProperty, IPropertyReader {
    // The entry objects included in this property.
    private final IProperty[] entries;

    public BreakSpeed(String path, JsonObject root, JsonArray nodes) {
    ...
    public void modifyBreakSpeed(BreakSpeedInfo breakSpeed) {
        for (IProperty entry : this.entries) {
            if (entry != null) {
                entry.modifyBreakSpeed(breakSpeed);
            }
        }
    }
    ...
    public IProperty readLine(String path, JsonObject root, int index, JsonElement node) {
        ...
        if (function.equals("set"))
            return new EntrySpeed(0, path, root, index, objNode, this);
        if (function.equals("add"))
            return new EntrySpeed(1, path, root, index, objNode, this);
        if (function.equals("mult"))
            return new EntrySpeed(2, path, root, index, objNode, this);
        if (function.equals("copy"))
            return new EntrySpeedCopy(path, root, index, objNode, this);
